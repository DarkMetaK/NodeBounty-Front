...
export function StatementItem({ data, owner }) {
  const generatePDF = () => {
    ...
    pdf.text(
      `Data da Transação: ${dayjs(data.transacao.dataTransacao).format(
        'DD/MM/YYYY',
      )}`,
      20,
      70,
    ...
    if (data.transacao.emissor && data.transacao.receptor) {
      ...
      pdf.text(
        `CPF: ${formatarCPF(data.transacao.emissor.cliente.cpf)}`,
        20,
        130,
      )
      ...
      pdf.text(
        `CPF: ${formatarCPF(data.transacao.receptor.cliente.cpf)}`,
        20,
        170,
      )
    } else {
  ...
  function formatarCPF(cpf) {
    ...
    const cpfAsteriscos = cpf.replace(
      /^(.{3})(.*)(.{2})$/,
      (_, first, middle, last) => {
  ...
  const getTransactionIcon = () => {
    if (data.transacao.emissor === null) {
      return (
        <img className={styles.svg} src={DepositIcon} alt="Ícone de Depósito" />
      )
    } else if (data.transacao.receptor === null) {
      return (
        <img className={styles.svg} src={WithdrawIcon} alt="Ícone de Saque" />
      )
    } else {
      return (
        <img
    ...
    <div className={styles.transacaoContainer}>
      <div className={styles.transacao}>
        <div className={styles.data}>
          {dayjs(data.transacao.dataTransacao).format('DD/MM/YYYY')}{' '}
        </div>
